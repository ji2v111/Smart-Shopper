...
class ApiService {
  // ── تغيير هذا الـ IP إلى IP جهازك عند التشغيل على جهاز حقيقي ──
  // Android Emulator  → http://10.0.2.2:8000 , 127.0.0.1:8000
  // iOS Simulator     → http://localhost:8000
  // جهاز حقيقي       → http://192.168.x.x:8000  (IP جهاز الكمبيوتر)
  //static const baseUrl = 'http://10.0.2.2:8000';
static const String baseUrl = "http://10.0.2.2:8000"; // Change 8000 to 5000 if needed
  // ── Session ─────────────────────────────────────────
  ...
  /// Headers مع Bearer token الصحيح
  static Future<Map<String, String>> _headers() async {
    final t = await getToken();
    return {
      'Content-Type': 'application/json',
      if (t != null) 'Authorization': t,   // Backend يستخدم raw token بدون Bearer
    };
  }
  ...
  // ── Image ────────────────────────────────────────────
  static Future<String> cropImage(File f) async {
    final t = await getToken();
    ...
    return d['cropped_image_b64'] as String;
  }
